...
class App:
    def __init__(self, window):
        ...
        self.potionp = Label(text=" 포션 : ", bg=self.bg_color["off"])
        self.potionp.grid(row=3, column=0, padx=3, pady=2)

        self.potion = Canvas(self.window, width=65, height=65, bg=self.bg_color["off"])
        self.potion.grid(row=3, column=1, columnspan=3)        
        ################################################################################################
        sep1 = ttk.Separator(self.window, orient="horizontal")
        sep1.grid(row=4, column=0, columnspan=4, sticky=NSEW)
        ...
        self.youtube = Button(self.window, text="Youtube", width=6, bg=self.bg_color["off"], relief="groove", command=lambda:webbrowser.open("https://mandloh.tistory.com/70"))
    ...
    def update(self):
        if self.win_hwnd:
            try:
                ...
                img_hp = self.win_get_image(x1 + 115, y1 + 38, x1 + 390, y1 + 60)
                ...
                img_mp = self.win_get_image(x1 + 115, y1 + 54, x1 + 390, y1 + 75)
                ...
                img_potion = self.win_get_image(x1 + 705, y1 + 965, x1 + 765, y1 + 1030)
                self.photo_potion = PIL.ImageTk.PhotoImage(image = PIL.Image.fromarray(img_potion))
                self.potion.create_image(0, 0, image = self.photo_potion, anchor = NW)
                img_potion = cv2.cvtColor(img_potion, cv2.COLOR_RGB2HSV)
