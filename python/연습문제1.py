# Q1. 홍길동 씨의 과목별 점수는 다음과 같다. 홍길동 씨의 평균 점수를 구해 보자.
'''
과목    점수
국어    80
영어    75
수학    55
'''
a = (80+75+55)/3
print(a)


# Q2. 자연수 13이 홀수인지 짝수인지 판별할 수 있는 방법에 대해 말해 보자.
a = 13
'''
나머지 구하는 연산자 %를 사용하여 1 or 0 에서 True or False 를 구분할 수 있다.
'''
a = 13 % 2
bool = bool(a)
print(bool)


# Q3. 홍길동 씨의 주민등록번호는 881120-1068234이다. 홍길동 씨의 주민등록번호를 연월일(YYYYMMDD)부분과 그 뒤의 숫자 부분으로 나누어 출력해 보자.
pin = "881120-1068234"

YYYYMMDD = str(19) + pin[0:6]
num = pin[7:]

print(YYYYMMDD) # 연월일 부분 출력
print(num) # 숫자 부분 출력


# Q4. 주민등록번호 뒷자리의 맨 첫 번쨰 숫자는 성별을 나타낸다. 주민등록번호에서 성별을 나타내는 숫자를 출력해 보자.
pin = "881120-1068234"
a = pin[7]
print(a)


# Q5. 다음과 같은 문자열 a:b:c:d 가 있다. 문자열의 replace 함수를 사용하여 a#b#c#d 로 바꿔서 출력해 보자.
a = "a:b:c:d"
b = a.replace(':','#')
print(b)    # 문자열 "a#b#c#d" 출력


# Q6. [1,3,5,4,2] 리스트를 [5,4,3,2,1]로 만들어 보자
a = [1,3,5,4,2]
a.sort()      # [1,2,3,4,5] 로 변경
a.reverse()      # [5,4,3,2,1] 로 변경
print(a)


# Q7. ['Life', 'is', 'too', 'short'] 리스트를 Lifr is too short 문자열로 만들어 출력해 보자.
a = ['Life', 'is', 'too', 'short']
result = (' '.join(a))
print(result)


# Q8. (1,2,3) 튜플에 값 4를 추가하여 (1,2,3,4)를 만들어 출력해 보자
a = (1,2,3)
a = a + (4,)
print(a)


# Q9. 다음과 같은 딕셔너리 a가 있다.
a = dict()
# a['name'] = 'python' # 1번
# a[('a',)] = 'python' # 2번
# a[[1]] = 'python' # 3번
# a[250] = 'python' # 4번
print(a)


# Q10. 딕셔너리 a에서 'B'에 해당하는 값을 추출해 보자.
a = {'A':90, 'B':80, 'C':70}
result = a.pop('B')
print(a)
print(result)


# Q11. a 리스트에서 중복 숫자를 제거해 보자.
a = {1,1,1,2,2,3,3,3,4,4,5}
aSet = tuple(a)
b = list(aSet)
print(b)


# Q12. 파이썬은 다음처럼 동일한 값에 여러 개의 변수를 선언할 수 있다. 
# 다음과 같이 a, b 변수를 선언한 후 a 의 두 번째 요소값을 변경하면 b 값은 어떻게 될까?
# 그리고 이런 결과가 나오는 이유에 대해 설명해 보자.
a = b = [1,2,3]
print(id(a),id(b))
a[1] = 4
print(b)

from copy import copy
b = copy(a)
b[1] = 5
print(a, b)
